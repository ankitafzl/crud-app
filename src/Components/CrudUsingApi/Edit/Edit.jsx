...
const Edit = (props1) => {
  console.log("hyy",props1);
  const paperStyle = { padding: '30px 20px', width: 400, margin: '20px auto' }
  const headerStyle = { margin: '0px' }
  const marginTop = { marginTop: '10px' }

  const updateValues = ({
    username: props1?.data?.username || '',
    email: props1?.data?.email || '',
    age: props1?.data?.age || '',
    gender: props1?.data?.gender || '',
    education: props1?.data?.education || '',
    subjects: props1?.data?.subjects || []
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await fetch(`https://crudcrud.com/api/3260088d0fad40e7a9de4a2f856d81e5/unicorns/${props1?.data._id}`, {
        method: "PUT",
